...
placing the HFSS design into a Circuit design as a subcomponent and by connecting
the lumped components to
its ports.
...
On the way back of the coupling, temperature information is handled differently for
HFSS and Circuit.
...
from pyaedt import Circuit, Hfss, Icepak, downloads
...
###############################################################################
# Download the project archive and save it to the temporary folder.

temp_dir = tempfile.TemporaryDirectory(suffix=".ansys")
project_path = downloads.download_file(
    "circuit_hfss_icepak",
    "Circuit-HFSS-Icepak-workflow.aedtz",
...
###############################################################################
# Set the maximum number of iterations. Increase this value in the range 4-6 to ensure that a
# self-consistent thermal and electrics solution.
max_iter = 3
