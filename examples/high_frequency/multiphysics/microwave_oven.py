...
parasolid_path = download_file(source="oven", name="gingerbread.x_t", local_path=working_dir.name)
oven_path = download_file(source="oven", name="microwave_oven.aedt", local_path=working_dir.name)
...
hfss = ansys.aedt.core.Hfss(version=AEDT_VERSION, project=oven_path, non_graphical=NG_MODE, new_desktop=True)
...
exc = ipk.assign_em_losses(
    design=hfss.design_name, setup=hfss.setups[0].name, sweep="LastAdaptive", map_frequency=hfss.setups[0].props["Frequency"], surface_objects=[], assignment=["glassBowl", ginger_bread.name]
)
...
ipk.assign_free_opening(assignment=ipk.modeler["ovenCavity"].top_face_y.id, flow_type="Velocity", velocity=["0m_per_sec", "-0.5m_per_sec", "0m_per_sec"])
ipk.assign_free_opening(assignment=ipk.modeler["ovenCavity"].bottom_face_y.id, flow_type="Pressure")
...
rot_cyl = ipk.modeler.create_cylinder(orientation="Z", origin=[158.75, 228.6, 0], radius=110, height=150, material="air", name="rotating_cylinder")
...
microwave_objects = ipk.post.export_model_obj(
    objects,
...
# +
report = AnsysReport(version=ipk.aedt_version_id, design_name=ipk.design_name, project_name=ipk.project_name)
report.create()
report.add_section()
...
report.add_image(path=os.path.join(working_dir.name, "ginger_bread_cookie.png"), caption="HFSS Design of Ansys Microwave Oven")
...
def generate_streamline(stop):
    def generate_case(quantity_in, field_time, assignment=["ovenCavity", "rotating_cylinder"]):
        ...
        air_ux_case = ipk.post.export_field_plot(plot_name=f1.name, output_dir=working_dir.name, file_format="case")
    ...
    mesh_ginger = generate_case(assignment=["plateauPainEpices_Unnamed_5"], quantity_in="Temperature", field_time=stop)
...
def generate_temps(stop):
    ...
    pl.plot(export_image_path=os.path.join(working_dir.name, f'{generate_unique_name("Temperature")}.jpg'), show=False)
    return pl
