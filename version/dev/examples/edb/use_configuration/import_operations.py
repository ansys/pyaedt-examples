...
# - **extent_defeature**. Simplifies geometry before applying cutout to aid meshing.
# Only applies to Conforming bounding box. Defaults to `0` (disabled).
...
# - **custom_extent_units**. Units of the custom extent points. Defaults to `"mm"`.
# Only valid if `custom_extent` is provided.
# - **include_partial_instances**. Includes padstacks with bounding boxes intersecting the custom shape.
# May slow down export. Only valid with `custom_extent` and `use_pyaedt_cutout`.
# - **keep_voids**. Whether to keep voids intersecting the cutout polygon. Defaults to `True`.
# Valid only if `custom_extent` is provided.
...
# - **expansion_factor**. Computes the maximum between dielectric thickness and trace width (for nets with ports)
# multiplied by this factor. Defaults to `0` (disabled). Works only with `use_pyaedt_cutout`.
# - **maximum_iterations**. Maximum number of iterations allowed for cutout search. Defaults to `10`.
# - **preserve_components_with_model**. Preserves all pins of components with associated models (Spice or NPort).
# Only applicable for PyAEDT cutouts (excluding point list).
# - **simple_pad_check**. Uses pad center for intersection detection instead of bounding box. Defaults to `True`.
# Bounding box method is slower and disables multithreading.
# - **keep_lines_as_path**. Keeps lines as `Path` instead of converting to `PolygonData`.
# Only works in Electronics Desktop (3D Layout). May cause issues in SiWave. Defaults to `False`.
# - **include_voids_in_extents**. Includes voids in the computed extent (for Conforming only).
# May affect performance. Defaults to `False`.
